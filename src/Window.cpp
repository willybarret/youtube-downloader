...
void Window::closeEvent(QCloseEvent *event) {
  ...
  if (downloader->state() == QProcess::NotRunning) {
    ...
    QMessageBox msg;
    msg.setText("¿Detener tarea y salir?");
...
Window::Window() {
  setWindowTitle("YouTube Downloader - Built with <3 by Willy");
...
void Window::initLayout() {
  ...
  single = new QRadioButton("Individual");
  multiple = new QRadioButton("Lista de reproducción");
  playlist_grp->addButton(single);
  playlist_grp->addButton(multiple);
  modeLayout->addWidget(new QLabel("Modo selecionado: "), 0, Qt::AlignCenter);
  ...
  layout2->addWidget(new QLabel("Opciones: "));
...
void Window::initButtons() {
  ...
  subs = new QCheckBox("Subtítulos (todos)");
...
bool Window::optionsOK(const QString &url_text, const QString &dir,
                       size_t youtube_url_pos) {
  if (url_text.isEmpty()) {
    status->setText("[ERROR] La URL se encuentra vacía.");
    return false;
  }
  if (dir.isEmpty()) {
    status->setText("[ERROR] Seleccioná un directorio de descarga.");
    return false;
  }
  if (single->isChecked() && youtube_url_pos != string::npos) {
    ...
    if (watch_found == string::npos && short_found == string::npos) {
      status->setText(
          "[ERROR] Esta URL no parece contener algún video.\n¿Quizás es una "
          "lista de reproducción? Cambiá al modo lista de reproducción e "
          "intentá de nuevo.");
      return false;
    }
  }
  if (multiple->isChecked()) {
    ...
    if (found == string::npos) {
      status->setText(
          "[ERROR] Esta URL no parece ser una lista de reproducción.\n¿Quizás "
          "es un video? Cambiá al modo individual e intentá de nuevo.");
      return false;
    }
  }
  return true;
...
QStringList Window::writeArgs(const QString &url_text, const QString &dir,
                              size_t youtube_url_pos,
                              size_t youtube_share_pos) {
  QStringList args =
      (QStringList() << url_text.toStdString().c_str() << "--paths"
                     << dir.toStdString().c_str() << "--no-color");
  string kbps;
  args << "--format";
  if (video->isChecked()) {
    ...
    if (youtube_url_pos != string::npos || youtube_share_pos != string::npos) {
      args << "ba[ext=m4a]" << "-x" << "--audio-format" << "mp3"
           << "--audio-quality";
    } else {
  ...
  if (video->isChecked() && subs->isChecked())
    args << "--write-subs" << "--sub-langs" << "all" << "--convert-subs"
         << "srt";
  ...
  if (multiple->isChecked()) {
    if (video->isChecked())
      o_template =
          "[Playlist] %(playlist)s/%(playlist_index)s - "
          "%(title)s/%(playlist_index)s - %(title)s [%(height)sp].%(ext)s";
    else
      o_template = "[Playlist] %(playlist)s/%(playlist_index)s - %(title)s [" +
                   kbps + "].%(ext)s";
  } else {
    if (video->isChecked())
...
void Window::copyCommand() {
  ...
  if (!optionsOK(url_text, dir, youtube_url))
    return;
  ...
  status->setText("[Mensaje] Comando copiado al portapapeles.");
}
...
void Window::startDownload() {
  if (downloader->state() == QProcess::NotRunning) {
    ...
    status->setText("Iniciando descarga...");
    repaint();
    ...
    connect(downloader, SIGNAL(readyReadStandardOutput()), this,
            SLOT(getOutput()));
    connect(downloader, SIGNAL(readyReadStandardError()), this,
            SLOT(getError()));
    connect(downloader, SIGNAL(finished(int, QProcess::ExitStatus)), this,
            SLOT(checkStatus()));
  }
}
...
void Window::checkStatus() {
  if (downloader->exitStatus() == QProcess::NormalExit) {
    if (atoi(percent) == 100) {
      status->setText("Tarea finalizada.");
      repaint();
    } else {
...
void Window::stopJob() {
  if (downloader->state() == QProcess::Running) {
    downloader->kill();
    status->setText("Tarea cancelada por el usuario.");
    progress->setValue(0);
  }
}
